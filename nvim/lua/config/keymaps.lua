...
vim.keymap.set("n", "<leader>cT", function()
  ...
  tmux_claude("Write tests for this code:\n\n" .. ctx, {
    system_prompt = "You are an expert test engineer. Write thorough, well-structured tests. Cover edge cases, error paths, and happy paths. Use the testing framework already in use in the project when possible.",
  })
...
vim.keymap.set("n", "<leader>cR", function()
  ...
  tmux_claude("Suggest refactoring improvements for this code:\n\n" .. ctx, {
    system_prompt = "You are an expert software engineer focused on code quality. Identify opportunities to simplify, reduce duplication, improve naming, and enhance readability. Prefer small, targeted improvements over large rewrites. Explain the rationale for each suggestion.",
  })
...
vim.keymap.set("n", "<leader>cS", function()
  ...
  tmux_claude("Audit this code for security vulnerabilities:\n\n" .. ctx, {
    system_prompt = "You are a security engineer performing a code audit. Look for injection vulnerabilities, buffer overflows, authentication/authorization issues, data exposure, race conditions, and other OWASP top 10 and CWE top 25 issues. Rate severity and suggest fixes.",
  })
...
vim.keymap.set("n", "<leader>cD", function()
  ...
  tmux_claude("Write documentation for this code:\n\n" .. ctx, {
    system_prompt = "You are a technical writer. Write clear, concise documentation including function/module purpose, parameters, return values, usage examples, and important caveats. Match the documentation style already used in the project.",
  })
...
vim.keymap.set("n", "<leader>cg", function()
  ...
  tmux_claude("Write a commit message for these changes:\n\n" .. diff, {
    system_prompt = "You are an expert at writing git commit messages. Write a concise, descriptive commit message following conventional commits style. Start with a short summary line, then add a blank line and bullet points for details if needed. Focus on why, not just what.",
  })
...
vim.keymap.set("n", "<leader>cP", function()
  ...
  tmux_claude(prompt, {
    system_prompt = "You are an expert at writing pull request descriptions. Include a summary of changes, motivation, testing done, and any notable design decisions. Use markdown formatting. Be thorough but concise.",
  })
...
vim.keymap.set("n", "<leader>ce", function()
  ...
  tmux_claude("Explain this code in detail:\n\n" .. ctx, {
    system_prompt = "You are a patient, thorough code explainer. Break down the code's logic step by step. Explain the purpose, how it works, any patterns or idioms used, and potential gotchas. Tailor your explanation assuming the reader is a competent developer who is new to this codebase.",
  })
end, { desc = "Claude: explain code" })

-- Claude as architect
vim.keymap.set("n", "<leader>cA", function()
  local tree = vim.fn.system("find . -type f -not -path './.git/*' -not -path './node_modules/*' -not -path './build*/*' | head -100")
  local ctx = file_context() or ""
  local prompt = string.format("Advise on architecture.\n\nProject structure:\n%s\n\n%s", tree, ctx)
  tmux_claude(prompt, {
    system_prompt = "You are a senior software architect. Analyze the project structure and code to provide architectural guidance. Consider separation of concerns, modularity, scalability, and maintainability. Suggest improvements while respecting existing patterns and constraints.",
  })
